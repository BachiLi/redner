...
def save_obj(shape,
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(vertices.shape[0]):
            f.write('v {} {} {}'.format(vertices[i, 0], vertices[i, 1], vertices[i, 2]))
        if uvs is not None:
            for i in range(uvs.shape[0]):
                if flip_tex_coords:
                    f.write('vt {} {}'.format(uvs[i, 0], 1 - uvs[i, 1]))
                else:
                    f.write('vt {} {}'.format(uvs[i, 0], uvs[i, 1]))
        if normals is not None:
            for i in range(normals.shape[0]):
                f.write('vn {} {} {}'.format(normals[i, 0], normals[i, 1], normals[i, 2]))
