...
with tf.device(pyredner.get_device_name()):
    mat_green = pyredner.Material(
        diffuse_reflectance = tf.Variable([0.35, 0.75, 0.35], dtype=tf.float32, use_resource=True))
    mat_red = pyredner.Material(
        diffuse_reflectance = tf.Variable([0.75, 0.35, 0.35], dtype=tf.float32, use_resource=True))
    ...
    tri0_vertices = tf.Variable(
        [[-1.7,1.0,0.0], [1.0,1.0,0.0], [-0.5,-1.0,0.0]], dtype=tf.float32, use_resource=True)
