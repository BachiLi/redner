...
with tf.device('/device:cpu:' + str(pyredner.get_cpu_device_id())):
    position = tf.Variable([0.0, 2.0, -4.0], dtype=tf.float32, use_resource=True)
    look_at = tf.Variable([0.0, -2.0, 0.0], dtype=tf.float32, use_resource=True)
...
with tf.device(pyredner.get_device_name()):
    mat_shiny = pyredner.Material(
        ...
        specular_reflectance = tf.Variable([1.0, 1.0, 1.0], dtype=tf.float32, use_resource=True),
        roughness = tf.Variable([0.0005], dtype=tf.float32, use_resource=True))
