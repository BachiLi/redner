...
# shapes[0].normals = pyredner.pyredner.compute_vertex_normal(shapes[0].vertices, shapes[0].indices)
# shapes[1].normals = pyredner.pyredner.compute_vertex_normal(shapes[1].vertices, shapes[1].indices)
...
pyredner.imwrite(diff_depth,
    'results/test_g_buffer/init_depth_diff.png')
pyredner.imwrite(diff_normal,
...
for t in range(200):
    ...
    optimizer.apply_gradients(
        zip(
            grads, 
            [translation_params, euler_angles])
        )
